          ...
          <div key={t.name} className="min-w-full px-1">
            <figure className="rounded-xl border border-gray-100 bg-white p-6 shadow-sm">
              <div className="flex items-start gap-4">
                <div className="h-12 w-12 rounded-full overflow-hidden ring-1 ring-gray-200">
                  <Image src={t.img} alt={t.name} width={48} height={48} className="h-12 w-12 object-cover" />
                </div>
...
function ImpactCarousel() {
  ...
  const cards = [
    { title: "Food Aid", copy: "Cooked meals and staples for families.", color: brand.orange },
    { title: "Women Enterprise", copy: "Skills, mentorship and microgrants.", color: brand.green },
    { title: "Healthcare", copy: "Hospital visits and essentials.", color: brand.blue },
    { title: "Education", copy: "Reading glasses and school support.", color: brand.black },
  ];
  return (
    <div className="relative">
      <div className="absolute -left-2 top-1/2 -translate-y-1/2 z-10">
        <button aria-label="Scroll left" onClick={() => scrollBy(-320)} className="rounded-full border border-gray-200 bg-white/90 px-3 py-2 shadow-sm hover:bg-white">‹</button>
      </div>
      <div className="absolute -right-2 top-1/2 -translate-y-1/2 z-10">
        <button aria-label="Scroll right" onClick={() => scrollBy(320)} className="rounded-full border border-gray-200 bg-white/90 px-3 py-2 shadow-sm hover:bg-white">›</button>
      </div>
      <div ref={containerRef} className="flex gap-4 overflow-x-auto scroll-smooth no-scrollbar pr-2" style={{ scrollbarWidth: "none" }}>
        {cards.map((c) => (
          <div key={c.title} className="min-w-[280px] rounded-xl border border-gray-100 bg-white p-5 shadow-sm">
